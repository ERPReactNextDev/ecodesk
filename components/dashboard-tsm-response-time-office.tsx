...
const TSMResponseTimeOfficeCard = forwardRef((_props: Props, ref) => {
  ...
  downloadCSV() {
    ...
    const totalRow = [
      ...
      formatHoursToHMS(avgTSMResponseTime),
    ...
    downloadStyledWorkbookFromCsv(csv, "tsm-response-time-office.xlsx", downloadOptions);
  },
}));
  ...
  return (
    <Card className="shadow-sm hover:shadow-md transition-shadow duration-200 border-border/50">
      <CardHeader className="flex flex-col md:flex-row md:items-center md:justify-between gap-2 pb-3">
        <CardTitle className="text-lg font-semibold">Average Response Time Per TSM (Office)</CardTitle>
      ...
      <CardContent className="pb-3">
        ...
        {!loading && !error && groupedManager.length === 0 && <p>No TSM found for Office sales.</p>}
      </CardContent>
    </Card>
  );
});

export default TSMResponseTimeOfficeCard;
