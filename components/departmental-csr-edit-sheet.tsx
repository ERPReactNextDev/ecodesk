...
interface Props {
  ...
  onClose: () => void;
  referenceid: string;
  record: any;              // ← the row being edited
  onSave: (r: any) => void;
}

export function DepartmentalEditSheet({
  open,
  onClose,
  referenceid,
  record,
...
useEffect(() => {
  ...
  const { _id, referenceid, ...formData } = record;
  setF(formData);
}, [record]);
...
if (
  ...
  return toast.error("Ticket dates are required");
    ...
    try {
      setSaving(true);
            const r = await fetch("/api/departmental-csr-edit-activity", {
            method: "PUT",
